...
**示例获取与验证方法**：`scripts/gen_scan_examples.py` 在真实 IDA 环境重跑可重新生成全部 95 个真实 findings（输出到 `data/scan_findings.json`）；**每个示例的最终 verdict 必须经 L4 动态验证后回填**，未验证的保持 NEEDS_DYNAMIC。
